...
- **F1.2.c — Sınırlandırılmış QA orkestrasyon çekirdeği tamamlandı:** `QaOrchestrator`, framework/model sağlayıcısı/tool ayrıntısı bilmeden yalnız deterministik plan üretir ve doğrular. Kapalı `OrchestrationStepType` kümesi dışındaki adımlar açıkça reddedilir. Plan sırası, yeni bir agent sistemi oluşturmadan mevcut `AgentRouting` üzerinden gelir; agent ve framework kullanılabilirliği mevcut registry'ler üzerinden kontrol edilir. Bu ilk adım mevcut generate/execute/controller yollarını devralmaz ve kalıcı dağıtık iş durumu saklamaz.
- **F1.2.c doğrulaması:** Orchestration paketinin satır kapsamı `%96,67` (`116/120`), branch kapsamı `%82,86`; Maven'da `%90` paket satır kapsam kapısıyla korunur. `docs/agent-orchestration-architecture.md` mevcut sınırı ve sonraki teslimatları tanımlar.
